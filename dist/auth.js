...
(function () {
  ...
  function codeLeft() { return Math.max(0, Math.ceil((_codeUntil - Date.now()) / 1000)); }
  function updateCodeFeedback() {
    var btn = document.getElementById('clSendCode'), msg = document.getElementById('clCodeStatus'), left = codeLeft();
    if (btn) {
      btn.disabled = _codeSending || left > 0;
      btn.textContent = _codeSending ? '发送中…' : (left > 0 ? '重新发送（' + left + 's）' : (_verification ? '重新发送验证码' : '获取验证码'));
    }
    if (msg) {
      msg.textContent = _codeMessage;
      msg.hidden = !_codeMessage;
    }
    if (!_codeSending && left <= 0 && _codeTimer) {
      clearInterval(_codeTimer); _codeTimer = null;
    }
  }
  function startCodeCountdown() {
    _codeUntil = Date.now() + 60000;
    if (_codeTimer) clearInterval(_codeTimer);
    _codeTimer = setInterval(updateCodeFeedback, 1000);
    updateCodeFeedback();
  }
    ...
    return ensureProfile(_uid)
      ...
      .then(function (remote) {
        ...
        return writeProfile(profileId, _uid).then(function () {
          ...
          if (!silent) notify('已同步到云端');
          return 'ok';
        });
      })
      .catch(function (e) {
        _error = errText(e); _status = 'error'; render();
        if (!silent) notify('同步暂未完成，记录已保存在本机，联网后会自动重试');
        return null;
      })
      .then(function (v) {
        _busy = false;
  ...
  function statusText() {
    ...
    if (_status === 'off') return '尚未配置云端同步';
    if (_status === 'login') return '尚未开启云端同步';
    if (_status === 'syncing') return '正在同步到云端…';
    if (_status === 'error') return '同步暂未完成' + (n ? ' · ' + n + ' 条记录等待同步' : '') + '，联网后自动重试';
    if (n) return n + ' 条记录等待同步';
    return '已同步到云端 · ' + when(st && st.lastSync);
  }
  function cloudBox() {
    var el = _mount && document.getElementById(_mount);
    if (!el) return;
    var left = codeLeft();
    var codeLabel = _codeSending ? '发送中…' : (left > 0 ? '重新发送（' + left + 's）' : (_verification ? '重新发送验证码' : '获取验证码'));
  ...
  function sendCode() {
    ...
    if (_codeSending || codeLeft() > 0) return;
    if (!/^1\d{10}$/.test(raw)) { _codeMessage = '请填写 11 位手机号'; updateCodeFeedback(); notify(_codeMessage); return; }
    _phone = raw; _codeSending = true; _codeMessage = '正在发送验证码…'; updateCodeFeedback();
    ensure().then(function () { return _auth.getVerification({ phone_number: phone(raw) }); })
      .then(function (info) {
        _verification = info; _codeSending = false;
        _codeMessage = '验证码已发送到 ' + maskPhone(raw) + '，请查收短信。';
        startCodeCountdown();
        notify('验证码已发送，请查收短信');
      })
      .catch(function (e) {
        _codeSending = false; _codeUntil = 0; _codeMessage = errText(e);
        updateCodeFeedback(); notify(_codeMessage);
      });
  }
  function login() {
    ...
    if (!/^\d{4,8}$/.test(code)) { notify('请填写短信里的验证码'); return; }
      ...
      .then(function (uid) {
        ...
        _uid = uid; _status = 'ok'; _codeMessage = ''; _codeUntil = 0;
        if (_codeTimer) { clearInterval(_codeTimer); _codeTimer = null; }
        render(); return syncNow();
  ...
  window.Cloud = {
    ...
    __test: function (app, uid) {
      ...
      if (uid) _uid = uid;
      return { syncNow: syncNow, listEvents: listEvents, sendCode: sendCode, updateCodeFeedback: updateCodeFeedback };
    },
  };
